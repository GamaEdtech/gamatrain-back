// Vendored from npm "mathml2omml" 0.5.0 (https://github.com/fiduswriter/mathml2omml, LGPL-3.0-or-later).
...
function stringify(buff, doc) {
  switch (doc.type) {
    case 'text':
      return buff + xmlEscapeText(doc.data)
    case 'tag': {
      const voidElement =
...
function walker(
  ...
  if (
    ...
    // Locally disabled: addScriptlevel() unshifts a new <m:argPr><m:scrLvl>...</m:scrLvl></m:argPr>
    // every time it's called, and it's called once per <mstyle scriptlevel> wrapper -- MathJax emits one
    // per invisible-spacing character (e.g. the "\," runs inside a \begin{gathered} piecewise
    // definition), so a single m:e ends up with one m:argPr per space instead of at most one. Confirmed
    // against real exam data (831 test[6]) with DocumentFormat.OpenXml's own OpenXmlValidator: (1) this
    // duplication is itself invalid ("The element has unexpected child element ... m:argPr"), and (2)
    // <m:scrLvl> is not even a valid child of <m:argPr> per the real schema regardless of count --
    // OpenXmlValidator expects only <m:argSz> there. scriptlevel is a cosmetic font-scaling hint, not
    // math content, so dropping it entirely is the safe fix rather than trying to emit it correctly.
