...
| `UserSubscriptionQuota` (`UserSubscriptionQuota.cs`) | `UserSubscriptionQuotas` | One allowance **bucket** for one `UserSubscription` — covers one feature, or several when pooled (see `UserSubscriptionQuotaFeature` below); `Limit`/`Description` snapshotted from the group's `SubscriptionPlanFeature.Limit`/`FeatureGroupDescription` at activation | `Id` (PK), `UserSubscriptionId`, `Limit` (int?, snapshotted; `NULL` = unlimited), `Used`, `Description` (string?, snapshotted; `NULL` for an unpooled bucket) | `UserSubscriptionId` → `UserSubscriptions` (**Cascade**). Remaining (`Limit - Used`, `NULL` if `Limit` is `NULL`) is computed, never stored. |
