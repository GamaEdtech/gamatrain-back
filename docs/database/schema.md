...
| `SubscriptionPlan` (`SubscriptionPlan.cs`) | `SubscriptionPlans` | A purchasable subscription tier — **definition only**, no price (moved to `SubscriptionPlanPrice`) | `Id` (PK), `Title`, `Polygon` (`geography` — availability region), `IsActive`, `Highlight`, `BillingInterval` (smart enum, carries a `Days` count + `CalculateEndDate` helper) | `CreationUserId`/`LastModifyUserId` → `ApplicationUsers` |
| `Feature` (`Feature.cs`) | `Features` | Data-driven catalog of quota-limited actions (e.g. `PastpaperDownload`) | `Id` (PK, int), `Code` (unique), `Name`, `Description`, `IsActive`, `CreationDate` | — (no FK; referenced by `SubscriptionPlanFeature`/`UserSubscriptionQuota`) |
| `SubscriptionPlanFeature` (`SubscriptionPlanFeature.cs`) | `SubscriptionPlanFeatures` | Per-plan quota limit for one feature | `Id` (PK), `SubscriptionPlanId`, `FeatureId`, `Limit` (int) | `SubscriptionPlanId` → `SubscriptionPlans` (**Cascade**), `FeatureId` → `Features` (NoAction). Unique `(SubscriptionPlanId, FeatureId)`. |
| `SubscriptionPlanPrice` (`SubscriptionPlanPrice.cs`) | `SubscriptionPlanPrices` | Regional price for a plan; `CountryCode = NULL` is the global default row | `Id` (PK), `SubscriptionPlanId`, `CountryCode` (nullable), `Currency` (smart enum), `Price` (precision 36,18) | `SubscriptionPlanId` → `SubscriptionPlans` (**Cascade**). Unique `(SubscriptionPlanId, CountryCode)` — SQL Server treats `NULL` as a distinct value, so this also enforces at most one default row per plan. |
| `SubscriptionPlanGatewayMapping` (`SubscriptionPlanGatewayMapping.cs`) | `SubscriptionPlanGatewayMappings` | External gateway Product/Price (or Plan) id for one regional price; **written by admin now, not yet consumed** — reserved for the native-recurring-billing phase | `Id` (PK), `SubscriptionPlanPriceId`, `Gateway` (smart enum), `ExternalProductId`, `ExternalPlanId` (nullable) | `SubscriptionPlanPriceId` → `SubscriptionPlanPrices` (**Cascade**). Unique `(SubscriptionPlanPriceId, Gateway)`. |
| `UserSubscription` (`UserSubscription.cs`) | `UserSubscriptions` | A user's purchase/enrollment in a plan for one billing period | `Id` (PK), `UserId`, `SubscriptionPlanId`, `Status` (smart enum: Pending/Active/Expired/Cancelled), `CreationDate`, `StartDate`/`ExpirationDate` (nullable, set on activation), `PricePaid` (precision 36,18, snapshot), `Currency` (snapshot) | `UserId` → `ApplicationUsers` (NoAction), `SubscriptionPlanId` → `SubscriptionPlans` (NoAction — blocks deleting a plan that was ever sold). Indexes `(UserId, Status)`, `(Status, ExpirationDate)`. |
| `UserSubscriptionQuota` (`UserSubscriptionQuota.cs`) | `UserSubscriptionQuotas` | Per-feature remaining allowance for one `UserSubscription`, snapshotted from `SubscriptionPlanFeature` at activation | `Id` (PK), `UserSubscriptionId`, `FeatureId`, `Limit` (snapshotted), `Used` | `UserSubscriptionId` → `UserSubscriptions` (**Cascade**), `FeatureId` → `Features` (NoAction). Unique `(UserSubscriptionId, FeatureId)`. Remaining (`Limit - Used`) is computed, never stored. |
