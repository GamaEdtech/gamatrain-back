...
| `SubscriptionPlanFeature` (`SubscriptionPlanFeature.cs`) | `SubscriptionPlanFeatures` | Per-plan quota limit for one feature | `Id` (PK), `SubscriptionPlanId`, `FeatureId`, `Limit` (int?, `NULL` = unlimited) | `SubscriptionPlanId` → `SubscriptionPlans` (**Cascade**), `FeatureId` → `Features` (NoAction). Unique `(SubscriptionPlanId, FeatureId)`. |
...
| `UserSubscriptionQuota` (`UserSubscriptionQuota.cs`) | `UserSubscriptionQuotas` | Per-feature remaining allowance for one `UserSubscription`, snapshotted from `SubscriptionPlanFeature` at activation | `Id` (PK), `UserSubscriptionId`, `FeatureId`, `Limit` (int?, snapshotted; `NULL` = unlimited), `Used` | `UserSubscriptionId` → `UserSubscriptions` (**Cascade**), `FeatureId` → `Features` (NoAction). Unique `(UserSubscriptionId, FeatureId)`. Remaining (`Limit - Used`, `NULL` if `Limit` is `NULL`) is computed, never stored. |
