...
| `SubscriptionPlanFeature` (`SubscriptionPlanFeature.cs`) | `SubscriptionPlanFeatures` | Per-plan quota limit for one feature — plan-wide, not price/interval-specific | `Id` (PK), `SubscriptionPlanId`, `FeatureId`, `Limit` (int?, `NULL` = unlimited) | `SubscriptionPlanId` → `SubscriptionPlans` (**Cascade**), `FeatureId` → `Features` (NoAction). Unique `(SubscriptionPlanId, FeatureId)`. |
...
| `UserSubscription` (`UserSubscription.cs`) | `UserSubscriptions` | A user's purchase/enrollment in a plan for one billing period | `Id` (PK), `UserId`, `SubscriptionPlanId`, `Status` (smart enum: Pending/Active/Expired/Cancelled), `CreationDate`, `StartDate`/`ExpirationDate` (nullable, set on activation), `PricePaid` (precision 36,18, snapshot), `Currency` (snapshot), `BillingInterval` (snapshot — which interval of the plan was purchased; used to compute `ExpirationDate` without a join back to the plan/price) | `UserId` → `ApplicationUsers` (NoAction), `SubscriptionPlanId` → `SubscriptionPlans` (NoAction — blocks deleting a plan that was ever sold). Indexes `(UserId, Status)`, `(Status, ExpirationDate)`. |
