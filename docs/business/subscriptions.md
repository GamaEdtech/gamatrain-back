...
- **A renewal that lands on a pending downgrade's boundary now records the *new* price, not the
  stale one (fixed 2026-09-14).** Found live in production: a user's ordinary renewal invoice
  (`billing_reason: subscription_cycle`) was genuinely charged $19 by Stripe (the downgraded
  price, per the gateway's own Subscription Schedule flipping at this exact boundary — see
  `PendingSwitchPricePaid` above), but the recorded `Payment.Amount` was $39, the *old*
  pre-downgrade price. Root cause: `HandleInvoicePaidAsync` read `UserSubscription.PricePaid` and
  inserted the `Payment` row **before** calling `RenewSubscriptionAsync` — the very call that
  applies the pending downgrade (`PricePaid` ← `PendingSwitchPricePaid`) to match the gateway's
  schedule. `HandleInvoicePaidAsync` now reads `PendingSwitchPricePaid` alongside `PricePaid` and
  records `PendingSwitchPricePaid ?? PricePaid` — i.e. whatever `RenewSubscriptionAsync` is about
  to apply, matching what the gateway actually billed. No equivalent gap for a pending *upgrade*:
  an upgrade applies (and bills) immediately via `HandlePlanChangeInvoicePaidAsync`, so nothing is
  ever left pending for it to race against.
