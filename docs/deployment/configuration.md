...
- `Core` — external "Core"/gama-api service base URLs (`Cdn`, `Url`, `Test`, `ExamResult`, `ExamInfo`, `Boards`, `ExamDetailsUrl`, `Login`, `Register`, `Recovery`, `GoogleAuth`), used by both the pre-existing Core integration and the temporary legacy-auth-bridge, plus `JwtSigningSecret` — the real HS256 key gama-api signs its JWTs with, required to cryptographically verify any legacy JWT presented to gamatrain-back (see `docs/api/authentication.md`). Empty by default in the tracked file; must be obtained from the gama-api team out-of-band and set via environment-specific secret configuration — every legacy-JWT code path fails closed until it's set. (`UserInfo` removed 2026-09-03 alongside the `tokens/old` endpoint it only backed.)
