...
| GET | `plans/{id:long}/features` | Get a plan's feature limits; each entry's `pooledFeatureCodes`/`featureGroupDescription` describe its shared quota, if pooled, else `featureDescription` describes the single feature | route: `id` | `IEnumerable<PlanFeatureViewModel>` |
