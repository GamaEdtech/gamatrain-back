...
| GET | `plans/{id:long}/features` | Get a plan's feature limits; `description` is already resolved server-side (the pooled bucket's description when `pooledFeatureCodes` is non-empty, otherwise the single feature's own) — no client-side fallback needed | route: `id` | `IEnumerable<PlanFeatureViewModel>` |
