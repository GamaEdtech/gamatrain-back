...
| POST | `plans/{id:long}/purchase` | Start a subscription purchase: resolves price server-side by plan + `BillingInterval` (never a client-supplied amount), creates a `Pending` `UserSubscription` + `Payment`, returns the gateway checkout URL. Rejects (`succeeded:false`, `Duplicate`) if the caller already has an `Active` subscription - use `subscriptions/me/switch` instead (fixed 2026-08-15, see `docs/business/subscriptions.md`) | User | route: `id` + `PurchaseSubscriptionRequestViewModel` (body: `Gateway`, `BillingInterval`) | `PurchaseSubscriptionResponseViewModel` |
