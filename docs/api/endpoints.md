...
| GET | `dashboard` | Get current user's dashboard payload — Phase 0: field-for-field proxy of gama-api's teacher/student dashboard (server picks which from the caller's `Group`), nothing added yet from this backend; `legacyDataAvailable: false` (all other fields null) if gama-api was unreachable for this caller — never fails the request outright. See `docs/business/identity-and-access.md`'s "User dashboard proxy" | User | none | `DashboardResponseViewModel` |
